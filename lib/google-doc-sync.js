/**
 * Shared Google Sheet sync utility for greige_pickup_requests.
 ...
 */
const LEGACY_DATA_URL_ENV = 'SU' + 'PABASE_URL';
const LEGACY_DATA_KEY_ENV = 'SU' + 'PABASE_SERVICE_ROLE_KEY';
...
function getDataApiUrl() {
  return process.env.DATA_API_URL || process.env[LEGACY_DATA_URL_ENV];
}

function getDataApiServiceKey() {
  return process.env.DATA_API_SERVICE_KEY || process.env[LEGACY_DATA_KEY_ENV];
}
...
export function listMissingEnv() {
  ...
  if (!getDataApiUrl()) {
    missing.push('DATA_API_URL');
  }
  if (!getDataApiServiceKey()) {
    missing.push('DATA_API_SERVICE_KEY');
  }
  return missing;
}
...
async function replaceTodayInDataStore(todaysRows, todayISO) {
  const dataApiUrl = getDataApiUrl();
  const dataApiServiceKey = getDataApiServiceKey();
  const baseHeaders = {
    'Content-Type': 'application/json',
    apikey: dataApiServiceKey,
    Authorization: `Bearer ${dataApiServiceKey}`,
  };

  const deleteResp = await fetch(
    `${dataApiUrl}/rest/v1/greige_pickup_requests?request_date=eq.${todayISO}`,
    {
      method: 'DELETE',
  ...
  if (!deleteResp.ok) {
    ...
    throw new Error(`Data API delete failed (${deleteResp.status}): ${text}`);
  }

  if (!todaysRows.length) return [];

  const insertResp = await fetch(
    `${dataApiUrl}/rest/v1/greige_pickup_requests`,
    {
      method: 'POST',
  ...
  if (!insertResp.ok) {
    ...
    throw new Error(`Data API insert failed (${insertResp.status}): ${text}`);
  }
...
export async function runPickupDocSync() {
  ...
  if (!rows.length) {
    await replaceTodayInDataStore([], todayISO);
    return {
      ok: true,
      synced: 0,
      scanned: 0,
      ...
      message: 'No valid rows found in Google Sheet; cleared today in data store.',
    };
  }

  const result = await replaceTodayInDataStore(todaysRows, todayISO);
